...
select has_table('public', 'rider_documents', 'rider documents are stored');

select ok(
  (select relrowsecurity from pg_class where relname = 'rider_documents'),
  'and row-level secured');

-- THE test on this table.
--
-- A rider PATCHing status='approved' onto their own licence returned HTTP 200
...
-- it. An unvetted rider could have gone online carrying passengers.
select ok(
  not has_column_privilege('authenticated', 'public.rider_documents', 'status', 'UPDATE'),
  'a rider cannot approve their own documents');
...
select ok(
  has_column_privilege('authenticated', 'public.rider_documents', 'storage_path', 'UPDATE'),
