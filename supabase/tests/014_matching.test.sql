...
select ('d0000000-0000-4000-8000-00000000000' || n)::uuid,
       case when n = 7 then 'passenger'::user_role else 'rider'::user_role end,
       'D' || n, '+25078894000' || n
  from generate_series(1,7) n;
...
select is(
  (select count(*)::int from public.find_candidate_riders(
     st_point(30.0619,-1.9441)::geography, 'moto', 2000, 10)),
  1,
  'exactly one rider survives every filter'
);

select is(
  (select rider_id from public.find_candidate_riders(
     st_point(30.0619,-1.9441)::geography, 'moto', 2000, 10)),
...
select ok(
  (select distance_m from public.find_candidate_riders(
...
select is(
  ...
  2,
  'widening the radius reaches the far rider'
);
...
select ok(
  not has_function_privilege('authenticated',
    'public.find_candidate_riders(geography,vehicle_class,integer,integer)', 'EXECUTE'),
  'passengers cannot enumerate nearby riders'
);
