// Spawns virtual riders around Kigali, sets them online, moves them on a
...
//
// Without the working half, a passenger books and watches "Finding you a rider"
...
// rider JWT, so the simulation cannot pass through a transition the product
// would refuse.
...
const RIDERS = arg("riders", 10);
const MINUTES = arg("minutes", 1);
const PASSIVE = process.argv.includes("--passive");
const TICK_MS = 5000;

// How long a simulated rider takes at each leg. Roughly a short Kigali hop,
// slow enough to watch the passenger's sheet morph through every stage.
...
const byId = new Map(riders.map((d) => [d.id, d]));
const idList = riders.map((d) => `'${d.id}'`).join(",");
...
for (const d of riders) d.jwt = mint(d.id);
...
console.log(`seeding ${RIDERS} riders…`);
...
psql(`insert into public.profiles (id,role,first_name,phone) values ${
  riders.map((d) => `('${d.id}','rider','${d.name}','${d.phone}')`).join(",")};`);
psql(`insert into public.riders (id,verification) values ${
  riders.map((d) => `('${d.id}','verified')`).join(",")};`);
// A plate is what a passenger actually looks for at the kerb, so a simulated rider
...
console.log(
  `${RIDERS} riders online (${PASSIVE ? "passive - they will NOT take trips" : "they will accept and complete trips"}).`,
);
console.log(`Moving for ${MINUTES} minute(s); Ctrl-C to stop.`);

const until = Date.now() + MINUTES * 60_000;

// Leaving simulated riders `online` poisons everything that runs next: they
// stay in rider_presence_dispatchable_idx, they are matched to real trips by
// find_candidate_riders for as long as their heartbeat looks fresh, and
// scripts/e2e-dispatch.mjs starts losing its "only eligible rider" check to
...
// tripId -> when this rider will take its next action. Kept in memory on
...
async function work() {
  ...
  const offerRows = psql(
    `select o.id || '|' || o.rider_id || '|' || o.trip_id
       from public.trip_offers o
      where o.outcome is null and o.rider_id in (${idList});`,
  );
  for (const line of offerRows.split("\n").filter(Boolean)) {
    const [offerId, riderId, tripId] = line.split("|");
    if (!due(`offer:${offerId}`, ACCEPT_AFTER_MS)) continue;
    const d = byId.get(riderId);
    if (!d) continue;
    ...
    if (r.body?.state === "accepted") {
      ...
      // The offer lapsed or another rider got there first. Nothing to do.
  ...
  const tripRows = psql(
    `select t.id || '|' || t.rider_id || '|' || t.state
       from public.trips t
      where t.rider_id in (${idList})
        and t.state in ('accepted','arrived','in_progress');`,
  );
  for (const line of tripRows.split("\n").filter(Boolean)) {
    const [tripId, riderId, state] = line.split("|");
    const d = byId.get(riderId);
    if (!d) continue;
    ...
    if (state === "accepted") {
      ...
      // Within the detour tolerance of the quote, so the passenger pays what they
...
const timer = setInterval(async () => {
  if (Date.now() > until) {
    clearInterval(timer);
    parkRiders();
    console.log("simulation finished. Riders left offline, funded and verified.");
