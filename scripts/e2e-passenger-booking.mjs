// Walks exactly the server calls the passenger booking screens make: authenticated
...
async function main() {
  console.log("\nSeeding a passenger and one nearby moto…");
  psql(`insert into auth.users (instance_id,id,aud,role,email,confirmation_token,recovery_token,email_change_token_new,email_change,created_at,updated_at) values
   ('00000000-0000-0000-0000-000000000000','${PASSENGER}','authenticated','authenticated','b.passenger.${suffix}@test.local','','','','',now(),now()),
  ...
  {
    const r = await call("/rest/v1/rpc/search_landmarks", passengerJwt, { p_query: "kim", p_limit: 5 });
    check("a signed-in passenger can search", r.status === 200, `got ${r.status}`);
  ...
  {
    const r = await call("/rest/v1/rpc/search_landmarks", passengerJwt, { p_query: "simba", p_limit: 10 });
    check("a mid-string query finds both Simba branches", r.body?.length === 2, `got ${r.body?.length}`);
  }
  {
    const r = await call("/rest/v1/rpc/search_landmarks", passengerJwt, { p_query: "   ", p_limit: 10 });
  ...
  {
    const cols = "id,state,rider_id,quoted_amount_rwf,pickup_label,dropoff_label";
    const res = await fetch(`${API}/rest/v1/trips?id=eq.${tripId}&select=${cols}`, {
      headers: { apikey: ANON, Authorization: `Bearer ${passengerJwt}` },
    });
    const rows = await res.json().catch(() => null);
    check("the passenger reads back exactly the columns the sheet renders",
    ...
    check("no rider is attached yet", rows?.[0]?.rider_id === null);
  }
  ...
  check("the sheet would read 'Finding you a rider'",
  ...
  check("the rider accepts", accepted.body?.state === "accepted", JSON.stringify(accepted.body));
  check("the sheet would now read 'Rider on the way'",
